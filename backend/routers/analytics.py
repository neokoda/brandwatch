...
from backend.schemas.analytics import KPIs, TrendPoint, GeoPoint, SourceBreakdown, AuthorStat, LanguageBreakdown, EmotionStat
...
async def kpis(
    ...
    result = await db.execute(
        select(
            ...
            func.avg(Mention.engagement_score).label("avg_eng"),
...
async def geographic(
    ...
    q = _account_mention_query(current_user.account_id, tracker_id).where(
        Mention.ingested_at >= since, Mention.region_code.isnot(None)
    )
    result = await db.execute(q)
    mentions = result.scalars().all()
...
async def authors(
    ...
    from collections import defaultdict
    authors_map: dict[str, dict] = defaultdict(lambda: {"count": 0, "sent_sum": 0.0, "eng_sum": 0.0, "influencer": False})
    for m in mentions:
...
async def languages(
    ...
    from collections import defaultdict
    langs: dict[str, dict] = defaultdict(lambda: {"total": 0, "pos": 0})
    for m in mentions:
