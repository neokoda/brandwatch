...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      analyticsApi.trends({ ...params, days: 30 }),
          ...
          <>
            {tab === "Trends" && (
              <div className="space-y-8">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-4">Sentiment over 30 days</p>
                  {trends.length > 0 ? <TrendChart data={trends} height={320} /> : <Empty />}
                </div>
              </div>
            )}
                ...
                {geo.length > 0 ? (
                  <ResponsiveContainer width="100%" height={360}>
                    <BarChart data={geo.slice(0, 20)} layout="vertical" margin={{ left: 20, right: 20 }}>
                      ...
                      <Bar dataKey="mention_count" fill="var(--foreground)" name="Mentions" radius={[0, 2, 2, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : <Empty />}
              </div>
            ...
            {tab === "Audience" && (
              <div className="space-y-6">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Top authors</p>
                {authors.length > 0 ? (
                  <div className="border border-border rounded-lg overflow-hidden">
                    {authors.slice(0, 15).map((a, i) => (
                      <div key={a.author_name} className={`flex items-center gap-4 px-4 py-3 text-sm ${i !== 0 ? "border-t border-border" : ""}`}>
                        ...
                        <span className="text-muted-foreground w-20 text-right">eng {a.total_engagement.toFixed(1)}</span>
                      </div>
                    ))}
                  </div>
                ) : <Empty />}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <ResponsiveContainer width="100%" height={300}>
                      <RadarChart data={emotions}>
                        ...
                        <Radar dataKey="share" fill="var(--foreground)" fillOpacity={0.15} stroke="var(--foreground)" strokeWidth={1.5} />
                        <Tooltip contentStyle={tooltipStyle} />
                      </RadarChart>
                    </ResponsiveContainer>
                    <div className="space-y-2">
                      {emotions.map((e) => (
                        <div key={e.emotion_label} className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-muted-foreground w-10 text-right">{(e.share * 100).toFixed(1)}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    {languages.slice(0, 15).map((l, i) => (
                      <div key={l.language_code} className={`flex items-center gap-4 px-4 py-3 text-sm ${i !== 0 ? "border-t border-border" : ""}`}>
                        ...
                        <div className="flex-1 bg-muted rounded-full h-1.5">
                          <div className="bg-foreground h-1.5 rounded-full" style={{ width: `${(l.positive_share * 100).toFixed(0)}%` }} />
                        </div>
                        <span className="text-muted-foreground">{l.mention_count.toLocaleString()} mentions</span>
                        <span className="text-positive text-xs">{(l.positive_share * 100).toFixed(0)}% positive</span>
                      </div>
                    ))}
                  </div>
                ) : <Empty />}
